...
/**
 * Loads `apps/api/.env.test` into `process.env`.
 *
 ...
 *
 * It is called from both `global-setup.ts` and `setup.ts` because Jest runs the
 * global setup in a separate process from the workers, so the two do not share
 * an environment.
 *
